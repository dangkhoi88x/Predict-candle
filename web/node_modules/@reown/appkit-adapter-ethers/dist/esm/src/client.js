...
import { BaseProvider, CoinbaseWalletProvider, EthersHelpersUtil, InjectedProvider, SafeProvider } from '@reown/appkit-utils/ethers';
...
export class EthersAdapter extends AdapterBlueprint {
    ...
    async createEthersConfig() {
        const { metadata, enableCoinbase, enableBaseAccount, enableInjected, enableEIP6963 } = OptionsController.state;
        if (!metadata) {
            return undefined;
        }
            ...
            if (connection) {
                ...
                if (connection.account) {
                    ...
                    if (!connector.provider && ethersProvider) {
                        ...
                        connector.provider = (await ethersProvider.getProvider());
                    }
    ...
    async getAccounts(params) {
        ...
        if (connection && connection.caipNetwork) {
            return {
                accounts: connection.accounts.map(({ address }) => CoreHelperUtil.createAccount({
                    caipAddress: `${connection?.caipNetwork?.caipNetworkId}:${address}`,
                    type: 'eoa'
                }))
            };
        }
        ...
        const caipNetwork = ChainController.getActiveCaipNetwork(this.namespace);
        return {
            accounts: caipNetwork
                ? accounts.map(account => CoreHelperUtil.createAccount({
